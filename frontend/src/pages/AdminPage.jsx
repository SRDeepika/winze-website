        ...
        <div style={styles.container}>
            <div style={styles.card}>
                ...
                <div style={styles.demoText}>
                    <p>Admin: admin/admin123 | Super Admin: superadmin/SuperAdmin@2024</p>
                </div>
            </div>
        </div>
    );
        ...
        <div style={{ display: 'flex', minHeight: '100vh' }}>
            ...
            <div style={styles.sidebar}>
                <div style={styles.sidebarHeader}>
                    <h2>⚡ Winze Admin</h2>
                    <p>{adminUsername} ({adminRole})</p>
                </div>
                <nav>
                    <button onClick={() => setActiveTab('dashboard')} style={{...styles.navBtn, background: activeTab === 'dashboard' ? '#667eea' : 'transparent'}}>📊 Dashboard</button>
                    <button onClick={() => setActiveTab('blogs')} style={{...styles.navBtn, background: activeTab === 'blogs' ? '#667eea' : 'transparent'}}>📝 Blogs</button>
                    <button onClick={() => setActiveTab('jobs')} style={{...styles.navBtn, background: activeTab === 'jobs' ? '#667eea' : 'transparent'}}>💼 Jobs</button>
                    <button onClick={() => setActiveTab('applications')} style={{...styles.navBtn, background: activeTab === 'applications' ? '#667eea' : 'transparent'}}>📋 Applications</button>
                    <button onClick={() => setActiveTab('quotes')} style={{...styles.navBtn, background: activeTab === 'quotes' ? '#667eea' : 'transparent'}}>📧 Quotes</button>
                    <button onClick={() => setActiveTab('socialLinks')} style={{...styles.navBtn, background: activeTab === 'socialLinks' ? '#667eea' : 'transparent'}}>🔗 Social Links</button>
                    <button onClick={() => setActiveTab('profile')} style={{...styles.navBtn, background: activeTab === 'profile' ? '#667eea' : 'transparent'}}>👤 Profile</button>
                    {isSuperAdmin && <button onClick={() => setActiveTab('users')} style={{...styles.navBtn, background: activeTab === 'users' ? '#667eea' : 'transparent'}}>👥 Users</button>}
                                                    ...
                                                    <tbody>
                                                        {clicks.map((click, idx) => (
                                                            <tr key={click.id} style={{ borderBottom: '1px solid #eee' }}>
                                                                ...
                                                                <td style={{ padding: '8px', wordBreak: 'break-all' }}>{click.link_url}</td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                                </table>
...
const styles = {
    ...
    card: { background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', borderRadius: '20px', padding: '40px', width: '100%', maxWidth: '400px', textAlign: 'center' },
